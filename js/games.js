...
const KSGames = {
    ...
    bindGameCards() {
        document.querySelectorAll('.game-card').forEach(card => {
            card.addEventListener('click', () => {
                /* ── TIMER GUARD ────────────────────────────────────────────
                   If the daily game timer has already expired, block the
                   game from opening and show the Time Over modal instead. */
                if (KSTimer.remainingSeconds <= 0) {
                    KSTimer._showTimeOverModal();
                    return; // ← early exit; game does NOT open
                }
    ...
    startGame(gameId) {
        ...
        /* Initialise non-repeating pools for pool-based games */
        if (typeof def.initPool === 'function') def.initPool();

        /* ── START TIMER ────────────────────────────────────────────
           The timer begins counting down the moment a game opens.
           If time is already 0, startTimer() will show the modal
           and the game arena will not appear (handled by the guard
           in bindGameCards, but double-checked here for safety). */
        KSTimer.startTimer();
    ...
    exitGame() {
        /* ── STOP TIMER ─────────────────────────────────────────────
           Pause the countdown and persist progress the moment the
           user leaves a game (← Back button or sidebar nav). */
        KSTimer.stopTimer();
